...
async def get_record(
    ...
    return RecordOut(
        ...
        notes_count=record.get("notes_count", 0),
...
async def update_record(
    ...
    return RecordOut(
        ...
        notes_count=updated.get("notes_count", 0),
        user_uuid=updated.get("user_uuid"),
...
async def list_notes(
    ...
    return [{"id": n["id"], "record_id": n["record_id"], "content": n["content"], "created_at": n["created_at"]} for n in notes]
...
async def create_note(
    ...
    note_body = NoteCreate(content=body["content"])
    record = crud_records.get_record_by_id(record_id)
    if record is None:
        raise HTTPException(status_code=404, detail="Registro no encontrado")
    note = crud_notes.create_note(record_id, note_body)
    crud_records.increment_notes_count(record_id)
    return {"id": note["id"], "record_id": note["record_id"], "content": note["content"], "created_at": note["created_at"]}
